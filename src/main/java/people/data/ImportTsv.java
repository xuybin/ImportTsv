...
public class ImportTsv extends Configured implements Tool {
  ...
  public final static String JOB_NAME_CONF_KEY = "replication.job.name";
  ...
  public final static String GENERATE_ROWKEY = "generate.rowkey";
  public final static String HMACMD5_SECRETKEY ="hmacmd5.secretKey";
  ...
  private static void usage(final String errorMsg) {
    ...
    String usage =
      ...
      "  -D" + JOB_NAME_CONF_KEY + "=jobName - use the specified replication job name for the import\n" +
  ...
  public int run(String[] args) throws Exception {
    ...
    if (null == getConf().get(MAPPER_CONF_KEY)) {
      ...
      Integer generate_rowkey_column_index=getConf().getInt(GENERATE_ROWKEY,-1);
      //System.err.println("generate_rowkey_column_index="+generate_rowkey_column_index);
      if(columns.length>0 && generate_rowkey_column_index!=-1 && (generate_rowkey_column_index<=0 || generate_rowkey_column_index>=columns.length)){
          usage("  -D" +GENERATE_ROWKEY+" vale must >0 and <("+getConf().get(COLUMNS_CONF_KEY)+").length");
          return -1;
      }else  if(columns.length>0 && !columns[0].equals(TsvParser.ROWKEY_COLUMN_SPEC)){
          usage("  -D" +GENERATE_ROWKEY+" already set,then must begin with -D"+COLUMNS_CONF_KEY+"="+TsvParser.ROWKEY_COLUMN_SPEC+",..othercolumn....");
          return -1;
      }
      String  hMacMD5Secretkey= getConf().get(HMACMD5_SECRETKEY, "");
      if(generate_rowkey_column_index==-1 && !hMacMD5Secretkey.isEmpty()){
          usage("  -D" +HMACMD5_SECRETKEY+" Must use with -D"+GENERATE_ROWKEY);
      }
